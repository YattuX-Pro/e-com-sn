...
interface SparePartImageUploadDialogProps {
  open: boolean
...
export function SparePartImageUploadDialog({ 
  open, 
  onOpenChange, 
  partId, 
  partName,
  ...
  currentImages = [],
  onUploadComplete 
}: SparePartImageUploadDialogProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (imagesToDelete.length > 0) {
        ...
        const deleteResponse = await fetch(`${API_URL}/spareparts/${partId}/images/delete`, {
          method: 'POST',
          headers: {
      ...
      if (mainImageFile || detailImageFiles.length > 0) {
        ...
        const uploadResponse = await fetch(`${API_URL}/spareparts/${partId}/images`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="text-primary">Gérer les images - {partName}</DialogTitle>
        </DialogHeader>
